...
class VLMadapter():
    ...
    def train(pathImgTrain, pathImgVal, pathImgTest, nnArchitecture, nnIsTrained, class_num, batch_size, nb_epoch,
        ...
        text_encoder = ImprovedDeepSeekTextEncoder(
            proj_dim=proj_dim,
            local_model_path='/.cache/huggingface/hub/models--deepseek-ai--deepseek-vl-7b-base/snapshots/main/'
        )
        ...
        # -------------------------
        model_file_name = '/result_archive/Office31_Amazon_deepseek/Model_path.pth'
        ...
        for epochID in range(0, nb_epoch):
            ...
            tokenizer = AutoTokenizer.from_pretrained(
                '/huggingface/hub/models--deepseek-ai--deepseek-vl-7b-base/snapshots/main/'
            )
            class_enc = tokenizer(
                prompts,
                padding='max_length',
    ...
    def test(pathImgTest, pathModel, nnArchitecture, class_num, nnIsTrained, batch_size,
        ...
        prompts = [prompt_template.format(name.replace("_", " ")) for name in class_names]
        tokenizer = AutoTokenizer.from_pretrained(
            '/.cache/huggingface/hub/models--deepseek-ai--deepseek-vl-7b-base/snapshots/main/'
        )
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
