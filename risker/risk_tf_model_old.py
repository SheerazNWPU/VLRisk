...
import numpy as np
from . import tf_learn_weights_old as tflearn
import logging
...
class RiskModel(object):
    ...
    def train(self, train_machine_probs, valida_machine_probs, test_machine_probs, train_machine_mul_probs, valida_machine_mul_probs, test_machine_mul_probs,
        ...
        self.prob_dist_mean, self.prob_dist_variance = sbf.calculate_similarity_interval_distributions(
            ...
            self.train_data.id_2_risk_labels,
        ...
        logging.info(self.prob_dist_mean.shape)
        for i in range(class_num):
            for j in range(1, cfg.interval_number_4_continuous_value):
                if self.prob_dist_mean[i][j] == -1:
                    self.prob_dist_mean[i][j] = self.prob_dist_mean[i][j-1]
        ...
        parameters = tflearn.fit(self.validation_data.machine_labels.reshape([self.validation_data.data_len, 1]),
                                 ...
                                 self.validation_data.get_risk_variance_X_discrete(),
                                 self.validation_data.get_risk_variance_X_continue(),
                                 ...
                                 self.validation_data.risk_activate.reshape([-1, self.class_num]),
                                 self.validation_data.risk_labels.reshape([self.validation_data.data_len, 1]),
                                 self.validation_data.risk_mul_labels.reshape(
                                     [self.validation_data.data_len, self.class_num]),
                                 self.train_data.mu_vector,
                                 self.prob_dist_mean,
                                 None,
                                 init_variance=None)
    ...
    def predict(self, test_machine_probs, test_machine_mul_probs):
        ...
        #                                           self.prob_dist_variance)
        #
        # np.save('test_label.npy', self.test_data.true_labels)
        # self.test_data = self.validation_data
        # -- Apply learned risk model to workload. --
        # self.test_data = self.validation_data

        # print( self.test_data.get_risk_mean_X_discrete())
        results = tflearn.predict(self.test_data.machine_labels.reshape([self.test_data.data_len, 1]),
                                  ...
                                  self.test_data.get_risk_variance_X_discrete(),
                                  self.test_data.get_risk_variance_X_continue(),
                                  ...
                                  self.test_data.risk_activate.reshape([-1, self.class_num]),
                                  self.rule_learn_weights,
                                  self.machine_learn_weights,
...
class RiskLoss(_Loss):
    ...
    def forward(self, machine_lables, rule_mus, machine_mus,
        ...
        Fr_alpha = my_truncated_normal_ppf(self.alpha, self.a, self.b, big_mu, big_sigma ** 2)
        Fr_alpha_bar = my_truncated_normal_ppf(1 - self.alpha, self.a, self.b, big_mu, big_sigma ** 2)
